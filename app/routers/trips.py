...
from app.pipeline.budget import estimate_daily_budget
...
async def plan(request: ExtractionRequest):
    ...
    # server-computed budget (World Bank PPP cost-of-living adjustment)
    ext.daily_estimate_usd = await estimate_daily_budget(
        ext.destination.country, ext.budget_level,
    )
